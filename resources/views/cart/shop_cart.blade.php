...
@section('content')
    <section class="text-gray-600 body-font">
        @if(Session::get('success'))
            <div
                class="bg-green-100 border border-green-400 text-green-500 px-4 py-3 rounded relative shadow-md lg:w-1/3 w-full mx-auto overflow-auto text-center"
                role="alert">
        ...
        @elseif(Session::get('delSuccess'))
            <div
                class="bg-red-100 border border-red-400 text-red-500 px-4 py-3 rounded relative shadow-md lg:w-1/3 w-full mx-auto overflow-auto text-center pt-6"
                role="alert">
                ...
                <table class="table-auto w-full text-left whitespace-no-wrap">
                    ...
                    @foreach($items as $item)
                        <tr @if($loop->even) class="bg-gray-100" @endif> {{-- Aqui acessei uma propriedade interna do foreach pra inserir a linha cinza nos pares por exemplo --}}
                            ...
                            <form action="{{ route('user.editShopCart') }}" method="POST" enctype="multipart/form-data">
                                <td class="px-4 py-3">{{ $item->quantity }}</td>
                                <td class="px-4 py-3 text-sm text-right space-x-3 text-gray-900">
                                    <button class="mt-3 text-blue-500 inline-flex items-center">
                                        <b>Alterar</b>
                                    </button>

                                    <form action="{{ route('user.delShopCart') }}" method="POST"
                                          enctype="multipart/form-data">
                                        <!--Estou fazendo dessa forma, pois o Carrinho nao tem um db, decidi usar uma lib externa para somar + conhecimento, porém pretendo fazer como fiz com produtos por exemplo.-->
                                        @csrf
                                        <input type="hidden" name="id" value="{{ $item->id}}">
                                        <button class="mt-3 text-red-400 inline-flex items-center"><b>Remover</b>
                                        </button>
                                    </form>
                                </td>
